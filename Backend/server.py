...
def create_school(body: CreateSchoolRequest, user_data: dict = Depends(require_super_admin)):
    now = datetime.now(tz=ZoneInfo(DEVICE_TIMEZONE))
    record = {"name": body.name, "admin_email": body.admin_email, "timezone": body.timezone, "status": "active", "enrollment_code": _generate_enrollment_code(), "created_at": now, "created_by": user_data["uid"]}
