...
if [[ -z "$SA_JSON_FILE" ]]; then
    ...
    warn "  start but every token mint will fail until you SCP the JSON to"
    warn "  /opt/dismissal/Backend/firebase-scanner-sa.json (mode 600, owner"
    warn "  dismissal:dismissal), then: sudo systemctl restart dismissal-scanner"
    info ""
fi
